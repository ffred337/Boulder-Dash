    ...
    public void mechanicsManager() throws IOException {
       ...
       for(int j = mapHeight-1; j >=0; j--){
           for(int i = 0; i < mapWidth; i++){
               ...
               /*If under a Rock and a Diamond we have an complete space
               else if((element[i][j] instanceof Rock || element[i][j] instanceof Diamond) && element[i][j+1] instanceof Dirt){
                   //If it's a Rock we don't fall
               }*/
               ...
               /*If a diamond is on another one and both have space on the left
                   ...
                   this.map.getMapElements()[i-1][j] = new Diamond(i-1,j);
                   ((Diamond)this.map.getMapElements()[i-1][j]).setFallen(true);
                   this.map.getMapElements()[i][j] = new Bgd(i,j);
               }
